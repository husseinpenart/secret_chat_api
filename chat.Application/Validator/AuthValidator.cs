...
namespace secre_chat_api.chat.Application.Validator
{

    public class AuthValidator : AbstractValidator<UserRegisterDto>
    {
        public AuthValidator()
        {
            RuleFor(x => x.IdentityName)
                   .NotEmpty().WithMessage(MessageDictionary.Uservalidation.identityRequirement.ToString());
            RuleFor(x => x.phoneNumber)
                   .NotEmpty().WithMessage(MessageDictionary.Uservalidation.PhoneRequirement.ToString())
                    .Length(11).WithMessage(MessageDictionary.Uservalidation.PhoneNumberLength)
                    .Must(phone => phone != null && phone.StartsWith("09"))
                    .WithMessage(MessageDictionary.Uservalidation.PhoneStartNumber);
            RuleFor(x => x.userName)
                   .NotEmpty().WithMessage(MessageDictionary.Uservalidation.userNameRequirement.ToString());
